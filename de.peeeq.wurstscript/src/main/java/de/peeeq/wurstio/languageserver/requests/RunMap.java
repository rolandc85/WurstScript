...
public class RunMap extends MapRequest {
  private final @Nullable String wc3Path;
  private File customTarget = null;

  public RunMap(
      ConfigProvider configProvider,
      WFile workspaceRoot,
      @Nullable String wc3Path,
      @Nullable File map,
  ...
  public Object execute(ModelManager modelManager) throws IOException {
    ...
    try {
      ...
      if (map != null && !map.exists()) {
        throw new RequestFailedException(
            MessageType.Error, map.getAbsolutePath() + " does not exist.");
      }
      ...
      if (testMap != null) {
        ...
        if (path.length() > 0) {
          ...
          if (wc3RunArgs == null) {
            if (getPatchVersion().compareTo(VERSION_1_31) < 0) {
              cmd.add("-window");
            } else {
              cmd.add("-windowmode");
              cmd.add("windowed");
          ...
          WLogger.info(fullCommand.toString());
  ...
  private void validateInstallationDirectory() {
    ...
    if (osName.contains("win")) {
      ...
      if (new File("/Applications/Warcraft III").exists()) {
        found = true;
      }
    if (!found) {
      throw new RequestFailedException(
          MessageType.Error, "Could not find Warcraft III installation!");
    }
  }

  private GameVersion getPatchVersion() {
    String osName = System.getProperty("os.name");
    if (osName.contains("win")) {
      return W3Utils.getWc3PatchVersion();
    } else if (osName.contains("Mac")) {
      return VERSION_1_31;
    }
    return null;
  ...
  /** Returns the executable for Warcraft III for starting maps since it changed with 1.28.3 */
  private File findGameExecutable() {
    String osName = System.getProperty("os.name");
    if (osName.contains("win")) {
      if (W3Utils.getWc3PatchVersion().compareTo(VERSION_1_29) < 0) {
        return (Stream.of(
                "war3.exe", "War3.exe", "WAR3.EXE", "Warcraft III.exe", "Frozen Throne.exe"))
            ...
            .orElseThrow(
                () ->
                    new RequestFailedException(
                        MessageType.Error,
                        "No warcraft executatble found in path '"
                            + wc3Path
                            + "'. \n"
      ...
      return new File(wc3Path + "/x86_64/Warcraft III.app/Contents/MacOS/Warcraft III");
    }
    throw new RequestFailedException(
        MessageType.Error,
        "No warcraft executatble found in path '"
  ...
  private String findMapDocumentPath(String testMapName, File myDocumentsFolder) {
    ...
    if (documentPath == null) {
      ...
      if (W3Utils.getWc3PatchVersion().compareTo(new GameVersion("1.27.9")) <= 0) {
        ...
        File oldFile =
            new File(wc3Path, "Maps" + File.separator + "Test" + File.separator + testMapName);
        if (oldFile.exists()) {
          if (!oldFile.delete()) {
